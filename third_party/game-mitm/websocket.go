...
func (p *ProxyServer) handleWebSocket(w http.ResponseWriter, r *http.Request, isSecure bool) {
	...
	// 构建目标URL（私网 IP 替换为公网游戏服务器）
	targetHost := r.Host
	if isPrivateIPHost(targetHost) {
		targetHost = gameServerPublicHost
	}
	targetURL := url.URL{
		Scheme:   scheme,
		Host:     targetHost,
	...
	// 私网 IP 替换时修正 Host 头，避免游戏服务器拒绝请求
	if isPrivateIPHost(r.Host) {
		requestHeader.Set("Host", gameServerPublicHost)
	}
